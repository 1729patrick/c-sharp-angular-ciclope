...
{
    public class TBL_Divida_ATControllerTest
    {
        #region Unit Test
        [Fact]
        public async Task Test_GetListDividaAT()
        {
            ...
            using (var context = new ApplicationDbContext(options))
            {
                ...
                context.SaveChanges();
                context.TBL_Divida_AT.AddRange(
                    new TBL_Divida_AT { Divida = 50, EmpresaId = 1 },
                    new TBL_Divida_AT { Divida = 30, EmpresaId = 1 },
                    new TBL_Divida_AT { Divida = 0, EmpresaId = 1 });
            ...
            using (var context = new ApplicationDbContext(options))
            {
                ...
                var controller = new TBL_Divida_ATController(context, manager);

                var result = await controller.Index(1);

                var viewResult = Assert.IsType<ViewResult>(result);
                var model = Assert.IsAssignableFrom<IEnumerable<TBL_Divida_AT>>(viewResult.ViewData.Model);
