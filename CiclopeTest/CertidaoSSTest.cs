    ...
    public class CertidaoSSTest
    {
        ...
        public async Task Test_GetList()
        {
            ...
            using (var context = new ApplicationDbContext(options))
            {
                ...
                context.TBL_Doc_CertidaoSS.AddRange(
                    new TBL_Doc_CertidaoSS { DataFimValidade = DateTime.Now, IdEmpresa = 1, LocalFicheiro = "/" },
                    new TBL_Doc_CertidaoSS { DataFimValidade = DateTime.Now, IdEmpresa = 1, LocalFicheiro = "/" },
                    new TBL_Doc_CertidaoSS { DataFimValidade = DateTime.Now, IdEmpresa = 1, LocalFicheiro = "/" });
            ...
            using (var context = new ApplicationDbContext(options))
            {
                ...
                var controller = new CertidaoSSController(context, manager);

                var result = await controller.Index(1);

                var viewResult = Assert.IsType<ViewResult>(result);
                var model = Assert.IsAssignableFrom<IEnumerable<TBL_Doc_CertidaoSS>>(viewResult.ViewData.Model);
        ...
        public async Task Test_GetDetails()
        {
            ...
            using (var context = new ApplicationDbContext(options))
            {
                ...
                var viewResult = Assert.IsType<ViewResult>(result);
                var model = Assert.IsAssignableFrom<TBL_Doc_CertidaoSS>(viewResult.ViewData.Model);
        ...
        public async Task Test_Update()
        {
            ...
            using (var context = new ApplicationDbContext(options))
            {
                ...
                var viewResult = Assert.IsType<Ciclope.Models.TBL_Doc_CertidaoSS>(okResult.Model);

                var dataFimValidade = DateTime.Now;
                viewResult.DataFimValidade = dataFimValidade;
                ...
                viewResult = Assert.IsType<Ciclope.Models.TBL_Doc_CertidaoSS>(okResult.Model);

                Assert.Equal(dataFimValidade, viewResult.DataFimValidade);
            }
        }
